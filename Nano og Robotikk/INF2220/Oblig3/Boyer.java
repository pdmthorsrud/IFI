...
public class Boyer{
	...
	public void startProgram(String needleFile, String haystackFile) throws Exception{
		...
		if(needle.length>haystack.length){
			System.out.println("The needle is bigger than the haystack.");
			System.out.println("... you screwed up. I'm exiting now");
			System.exit(0);
		}

		ArrayList<Integer> indexes = searchForNeedle(needle, haystack);
		if(indexes!=null){
			System.out.println("\n The indexes at which '" + new String(needle) + "' occur are as follows: ");
			for(int e: indexes){
				System.out.println(" " + e + " || String: " + returnMatchedWord(e, needle.length, haystack));
			}
			System.out.println("");
		}else{
			System.out.println(" There are no strings in haystack that matches '" + new String(needle) + "'");
		}
	}	
